...
const { UnauthenticatedError } = require('sense-js-sdk');
...
class SenseDevice extends Homey.Device {
  async onInit() {
    ...
    this.client = this.homey.app.acquireSenseClient(this.monitorId, storedSession, createSdkLogger(this));
    ...
    this.client.emitter.on('reconnectFailed', this.handleReconnectFailed);
  ...
  async teardown() {
    ...
    this.client.emitter.off('reconnectFailed', this.handleReconnectFailed);

    // The client is shared with the sibling device for this monitor; the app
    // stops its websocket once the last device lets go.
    this.homey.app.releaseSenseClient(this.monitorId);
    this.client = null;
  }
  ...
  // The patched SDK reports a failed websocket reconnect here instead of
  // dying on an unhandled rejection. Auth failures need a re-pair, so stop;
  // anything else (an API blip at the ~16 minute socket cycle) retries.
  handleReconnectFailed(err) {
    this.handleApiError(err, 'Could not reconnect real-time updates')
      .catch((e) => this.error('Could not report reconnect failure:', e));

    if (this.isAuthFailure(err)) return;

    if (this.reconnectTimer) this.homey.clearTimeout(this.reconnectTimer);
    this.reconnectTimer = this.homey.setTimeout(() => {
      this.reconnectTimer = null;
      this.connectRealtime().catch((e) => this.error('Reconnect attempt failed:', e));
    }, MINUTE);
  }
  ...
  async setPower(capability, watts) {
    ...
    // Homey disables Flows that trigger too often, and a Flow on "power
    // changed" would fire on every write. The stream keeps ticking, so a
    // skipped value is superseded within a second.
    const throttleMs = (this.getSetting('liveUpdateSeconds') ?? 10) * 1000;
    const now = Date.now();
    this.lastPowerWrite ??= {};
    if (now - (this.lastPowerWrite[capability] ?? 0) < throttleMs) return;

    this.lastPowerWrite[capability] = now;
  ...
  // The SDK throws UnauthenticatedError when it has no session, but a revoked
  // session surfaces as a plain SenseApiError carrying the 401.
  isAuthFailure(err) {
    return err instanceof UnauthenticatedError || err?.status === 401;
  }
